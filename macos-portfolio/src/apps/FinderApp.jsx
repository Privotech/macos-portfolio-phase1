  ...
  const fileSystem = {
    "About Me": [
      ...
      },
      {
        ...
        type: "text",
        content:
          "Hi! I'm Privotech, a passionate Software Engineer and UI Designer dedicated to building robust and beautiful web experiences.",
        desc: "Short Biography",
      },
    ],
    Skills: [
      {
    ...
    Experience: [
      ...
      {
        name: "creative_agency.md",
        type: "text",
        content:
          "# Web Developer\n**2018 - 2021**\n- Built 20+ custom client websites.\n- Specialized in React and Vue.",
        desc: "Previous Role",
      },
    ],
    Certificates: [
      {
        name: "aws_certified.png",
        type: "image",
        content:
          "https://images.unsplash.com/photo-1523289217630-0dd16184af8e?w=400&h=300&fit=crop",
        desc: "AWS Solutions Architect",
      },
      {
        name: "meta_frontend.png",
        type: "image",
        content:
          "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?w=400&h=300&fit=crop",
        desc: "Meta Professional Certificate",
      },
    ],
    Resume: [
      {
        name: "privotech_resume.pdf",
        type: "pdf",
        content:
          "View full resume to see comprehensive details of my career history, education, and technical expertise.",
        desc: "Official PDF Resume",
      },
    ],
  };
  ...
  return (
    <div className="w-full h-full flex bg-[#1e1e1e] text-white overflow-hidden">
      <div className="w-48 bg-white/5 border-r border-white/10 flex flex-col py-2 flex-shrink-0">
        <div className="px-4 py-2 text-xs font-semibold text-white/50 mb-1">
          Favorites
        </div>
              ...
              return (
                ...
                >
                  <div className="w-16 h-16 flex items-center justify-center">
                    {file.type === "image" ? (
                      <FaImage className="text-5xl text-blue-300 drop-shadow-md" />
                    ) : file.type === "code" ? (
                      <FaCode className="text-5xl text-yellow-300 drop-shadow-md" />
                    ) : file.type === "pdf" ? (
                      <FaFileAlt className="text-5xl text-red-400 drop-shadow-md" />
                    ) : (
                      <FaFileAlt className="text-5xl text-white/80 drop-shadow-md" />
                    )}
                  </div>
                  <span
                    className={`text-xs text-center font-medium px-1.5 py-0.5 rounded truncate w-full ${
                      isSelected
                        ? "bg-blue-500 text-white"
                        : "group-hover:bg-blue-500/50 text-white/80"
                    }`}
                  >
                    {file.name}
                  </span>
            ...
            <div className="w-64 bg-white/5 border-l border-white/10 p-6 flex flex-col items-center overflow-y-auto">
              {selectedFile.type === "image" ? (
                <img
                  ...
                  className="w-32 h-32 object-cover rounded-xl shadow-lg mb-4 border border-white/10"
                />
              ) : (
                <div className="w-32 h-32 bg-white/10 rounded-xl shadow-lg mb-4 flex items-center justify-center border border-white/10">
                  {selectedFile.type === "code" ? (
                    ...
                    <FaFileAlt className="text-5xl text-white/50" />
                  )}
                </div>
              )}

              <h3 className="text-lg font-bold text-center mb-1 truncate w-full">
                {selectedFile.name}
              </h3>
