...
const MusicApp = () => {
  ...
  const playlist = [
    ...
    },
    {
      ...
      album: "Retro Future",
      duration: "3:45",
      cover: "https://images.unsplash.com/photo-1618331835717-801e976710b2?w=100&h=100&fit=crop",
  ...
  const playPrev = () => {
    setActiveSongIndex((prev) => (prev - 1 + playlist.length) % playlist.length);
    setIsPlaying(true);
  };
    ...
    <div className="w-full h-full flex flex-col bg-[#1e1e1e] text-white">
      ...
      <div className="h-16 bg-white/5 border-b border-white/10 flex items-center px-4 justify-between flex-shrink-0">
        <div className="flex gap-4 text-white/80">
          <FaStepBackward className="text-xl cursor-pointer hover:text-white transition-colors" onClick={playPrev} />
          <button onClick={togglePlay} className="focus:outline-none">
            {isPlaying ? <FaPause className="text-xl hover:text-white transition-colors" /> : <FaPlay className="text-xl hover:text-white transition-colors" />}
          </button>
          <FaStepForward className="text-xl cursor-pointer hover:text-white transition-colors" onClick={playNext} />
        </div>
        <div className="flex-1 max-w-md mx-8 bg-black/40 border border-white/5 rounded-md h-12 flex flex-col justify-center px-4 relative overflow-hidden group">
          <div className="flex items-center gap-3 relative z-10">
            <img src={currentSong.cover} className="w-8 h-8 rounded object-cover shadow-md" alt="Album" />
            <div className="flex flex-col justify-center flex-1">
              <span className="text-xs font-bold truncate">{currentSong.title}</span>
              <span className="text-[10px] text-white/50 truncate">{currentSong.artist}  {currentSong.album}</span>
            </div>
          </div>
          <input 
            type="range" 
          ...
          />
          <div className="absolute bottom-0 left-0 h-1 bg-blue-500 pointer-events-none transition-all duration-100 ease-linear" style={{ width: `${progress}%` }}></div>
        </div>
        <div className="flex items-center gap-3 text-white/60 w-32">
          <button onClick={() => setIsMuted(!isMuted)}>
            {isMuted || volume === 0 ? <FaVolumeMute className="hover:text-white" /> : <FaVolumeUp className="hover:text-white" />}
          </button>
          <input 
            type="range" 
          ...
          <div className="w-full text-left">
            ...
            {playlist.map((song, idx) => {
              ...
              return (
                <div 
                  key={idx} 
                  onDoubleClick={() => { setActiveSongIndex(idx); setIsPlaying(true); }}
                ...
                >
                  <div className="w-10 text-center text-white/40 group-hover:hidden">
                    {isCurrent && isPlaying ? <FaMusic className="mx-auto text-xs text-blue-400 animate-pulse" /> : idx + 1}
                  </div>
                  <div className="w-10 text-center hidden group-hover:block" onClick={() => { setActiveSongIndex(idx); setIsPlaying(true); }}>
                    <FaPlay className="mx-auto text-xs text-white" />
                  </div>
                  <div className={`flex-1 font-medium flex items-center gap-3 ${isCurrent ? 'text-blue-400' : 'text-white'}`}>
                    <img src={song.cover} className="w-8 h-8 rounded object-cover shadow-sm" alt="cover" />
                    <span className="truncate">{song.title}</span>
                  </div>
                  <div className="w-1/4 text-white/60 truncate hidden sm:block">{song.artist}</div>
                  <div className="w-1/4 text-white/60 truncate hidden md:block">{song.album}</div>
                  <div className="w-16 text-right text-white/60 hidden sm:block">{song.duration}</div>
                </div>
              );
            })}
          </div>
