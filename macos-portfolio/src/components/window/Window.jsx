...
const Window = ({ windowState }) => {
  ...
  // Track initial position for standard size
  const [position, setPosition] = useState({ x: 50, y: 50 });
  const [size, setSize] = useState({ width: 800, height: 500 });

  useEffect(() => {
    // Stagger initial positions slightly based on zIndex for new windows
  ...
  const windowStyle = isMaximized
    ? { top: 28, left: 0, width: '100vw', height: 'calc(100vh - 28px - 70px)' } // Accounts for menu bar and dock
    ...
    >
      <div 
        ...
        style={{
          zIndex,
          ...windowStyle,
          display: isMinimized ? 'none' : 'block' // Real hide if minimized
        }}
